# Requirements: Cloud Native Days France Website — v1.0 (ARCHIVED)

**Defined:** 2026-04-11
**Shipped:** 2026-04-13
**Status:** SHIPPED — 48/49 satisfied, 1 deferred to `cnd-platform` repo
**Audit:** passed (`.planning/v1.0-MILESTONE-AUDIT.md`, commit f21d19a)
...
- [x] **FNDN-07**: Kubernetes deployment manifests (Deployment, Service, Ingress) — **Deferred to cnd-platform** (GitOps split — out of scope for this repo by design)
...
## v2 Requirements (deferred for future milestone planning)
...
| FNDN-07 | Deferred to cnd-platform repo (GitOps) | Deferred |
...
- Unmapped: 0
- **Complete: 48**
- **Deferred: 1** (FNDN-07 — owned by `cnd-platform` repo)
- **Pending: 0**

## Outcomes

**Validated as-shipped** (functionally exercised in production-grade build, with content rendered through CSV pipelines and Zod-validated collections):

- All HERO-* (live countdown, CTA, key numbers)
- All SCHD-* (filtering, parallel-track timeline, localStorage bookmarks, iCal export, mobile view, Open Feedback deep links — Phase 8 UAT 5/5)
- All SPKR-*, SPNS-*, TEAM-*, VENU-* (rendered from CSV with bilingual fallback)
- All EVNT-* (runtime TARGET_DATE pattern proven via Phase 08 Stitch-first gate, 6/6 dimensions)
- All META-* (sitemap, hreflang, JSON-LD Event schema, OG/Twitter cards verified at build)
- DSGN-01..06, FNDN-01..06 (design tokens flow from DESIGN.md to Tailwind 4 `@theme`; 9.5MB Docker image)

**Validated by automated tests** (build-time + unit):

- FNDN-04 schemas: Zod validation gates the build
- SPKR-02/03 cross-references: `tests/build/speaker-profile.test.ts` and `speaker-talks.test.ts` (after Phase 13 cleanup)

**Carry-over for v1.1 visual UAT** (functionality shipped, subjective visual validation pending):

- 7 phases in `human_needed` UAT status: 01, 02, 03, 04, 05, 09, 11 (~18 subjective visual tests). Only Phase 08 UAT is fully complete.

**Deferred (out of scope by design):**

- **FNDN-07**: Kubernetes manifests live in the `cnd-platform` GitOps repo. This repo's responsibility ends at the Docker image; cnd-platform consumes it. Not a gap — an architectural split.

---
*Requirements defined: 2026-04-11. Milestone shipped: 2026-04-13.*

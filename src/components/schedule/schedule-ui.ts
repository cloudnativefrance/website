...
import { findClashes, substituteClashLabel } from "@/lib/agenda";
  ...
  function refreshAgenda(): void {
    ...
    const clashes = findClashes(
      picked.map((c) => ({
        ...
        // NaN, not 0, for a missing attribute: `Number(null ?? "0")` is 0,
        // which makes end === start and every overlap test false — the agenda
        // would report "no conflicts" forever rather than fail visibly.
    ...
    picked.forEach((card) => {
      ...
      const clashHtml = (clashes.get(id ?? "") ?? [])
        ...
        .map((other) => {
          // One pass over the template: sequential `.replace` with string
          // arguments would let a `$&` in a Pretalx title rewrite the output,
          // and would substitute into text the first call just inserted.
